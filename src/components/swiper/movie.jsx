...
export default function MovieSwiper() {
    ...
    useEffect(() => {
        fetch(
            `https://api.themoviedb.org/3/trending/all/week?api_key=${process.env.NEXT_PUBLIC_SLIDE_API_KEY}&language=en-US
`
        )
            .then((res) => res.json())
            .then((data) => {
        ...
        <>
            ...
            >
                {Array.isArray(section) && section.length > 0 ? (
                    section.map(
                        ({ id, poster_path, title, release_date, vote_average, name, first_air_date }, index) => (
                            <SwiperSlide key={id} /* className="w-[400px] sm:w-[300px] md:w-[350px]" */>
                                ...
                                <div className=" text-white font-bold z-20 truncate">{title || name}</div>
