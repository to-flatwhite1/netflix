...
export default function TrendingSwiper() {
    ...
    useEffect(() => {
        fetch(
            `https://api.themoviedb.org/3/tv/top_rated?api_key=${process.env.NEXT_PUBLIC_SLIDE_API_KEY}&language=ko-KR&page=1
            `
            /* `https://api.themoviedb.org/3/trending/movie/day?api_key=${process.env.NEXT_PUBLIC_SLIDE_API_KEY}&language=en-US` */
        )
            .then((res) => res.json())
            .then((data) => {
        ...
        <>
            ...
            >
                {Array.isArray(section) && section.length > 0 ? (
                    section.map(({ id, poster_path, name, first_air_date, vote_average }, index) => (
                        <SwiperSlide key={id} /* className="w-[400px] sm:w-[300px] md:w-[350px]" */>
                            ...
                            <div className=" text-white font-bold z-20 truncate">{name}</div>
